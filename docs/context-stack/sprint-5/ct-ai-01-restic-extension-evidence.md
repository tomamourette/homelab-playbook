...
5. ~~Role enhancement: support file paths via `[ -e ]` in the backup script's per-path loop (would unblock backing up `/etc/cost-cap.env` etc. directly)~~ **RESOLVED 2026-04-27** — homelab-infra commit `6aaf571` switched the gate to `[ -e ]` (covers files + directories + symlinks) and updated the warn message accordingly. Verified via `--check --diff` against all 5 backup_hosts (ct-docker-01, ct-media-01, ct-sparkle-cps, ct-dev-homelab, ct-ai-01) with zero failures and per-host diffs limited to the gate change. ct-ai-01 host_vars extended in the same commit to add the previously-omitted loose files: `/etc/cost-cap.env`, `/etc/cron.d/cost-cap`, `/usr/local/bin/cost-cap.sh`, `/var/log/cost-cap.log`, `/etc/systemd/system/litellm-gateway.service`, `/etc/systemd/system/gemma-hybrid-proxy.service`. Deploy not run as part of this story; next `deploy-restic.yml` run will re-render the script and pick up the loose files into ct-ai-01's backup set.
